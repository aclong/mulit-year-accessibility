...
do
#catching errors for october and january
    for month_string in 01 10
    do
        #check that date is before today
        today=`date +"%Y%m"`
        if [[ ${year}${month_string} -ge today ]]; then
            continue
        elif [[ ${year} -eq 2020 ]] && [[ ${month_string} -eq 01 ]]; then
            continue
        fi
        ...
        osm_full_filepath_sameyr="/home/ucfnacl/DATA/dynamic_accessibility/data/osm_data/west-midlands-${osm_year:2:4}0101.osm.pbf"
        echo "osm 2 digit year ${osm_year:2:4}"
        echo "full filepath ${osm_full_filepath_sameyr}"
        #check if the particular year you want exists as data if not select year before
        if [ -f $osm_full_filepath_sameyr ]; then
            osm_full_filepath=$osm_full_filepath_sameyr
            #echo "then copy ${osm_full_filepath} ${this_otp_dir}/"
        else
            osm_minus1_year=$[osm_year-1]
            osm_full_filepath="/home/ucfnacl/DATA/dynamic_accessibility/data/osm_data/west-midlands-${osm_minus1_year:2:4}0101.osm.pbf"
        fi

        #copy the osm data over
        ...
        #2g not enough for some - going to use 4g instead
        java -Xmx4G -d64 -server -jar ../otp-1.2.0-shaded.jar --basePath . --build .
        ...
        echo "GTFS data from ${gtfs_fullfilepath}" >> data_sources.txt
        echo "OSM data from ${osm_full_filepath} ish" >> data_sources.txt
